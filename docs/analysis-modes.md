...
3 モードは対象を全件ロードして描画するため、巨大アプリでは破綻する。これを補う横断的な検索システムを
別途用意している（`/search`）。検索窓・種別ファセットでの絞り込みと、グラフDBへの Cypher 直接クエリの
両方に対応し、全件ロードを避けて境界付き（ページング・近傍）で探索する。詳細は
[`search-system.md`](search-system.md) を参照。
